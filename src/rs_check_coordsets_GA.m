...
% Checks consistency of datasets and gets available stimuli, dimensions, and
% typenames. Number and names of stimuli must agree, and dimensions listed
% must agree with dimensions of coordinate arrays. Can also be used if `ds`,
% `sas`, `sets` are singleton cells, in which case it checks internal consistency.
%
% Args:
...
%     and `sets` (cell array of additional metadata structures).
%   opts (struct): Options, with fields `if_warn` (int, default 0, set to 1
%     to display warnings), `if_checkorder` (int, default 1, if set the order
%     of entries in `typenames` must match), and `set_num_offset` (int, number
%     to add to set number in warnings).
%
% Returns:
%   check (struct): Consistency check results, with fields `warnings` (strvcat
%     of warning strings), `warn_bad` (int, count of serious warnings that will
%     prevent processing), `nsets` (int, number of datasets), `nstims_each`
%     (array of shape `(1,nsets)` with number of stimuli in each dataset),
%     `dim_list_each` (cell array of shape `(1,nsets)` with list of dimensions
%     available in each dataset), `dim_list_union` (union of `dim_list_each`),
%     `dim_list_inter` (intersection of `dim_list_each`), `typenames_each`
%     (cell array of shape `(1,nsets)` with typenames in each dataset in
%     original order), `typenames_union` (union of `typenames_each`,
%     alphabetized), and `typenames_inter` (cell array of shape `(1,nsets)`
%     with intersection of `typenames_each`, alphabetized).
%   opts_used (struct): Options used.
%
% See also:
%   RS_KNIT_COORDSETS, RS_ALIGN_COORDSETS.
%
if (nargin<=1)
    opts=struct;
end
