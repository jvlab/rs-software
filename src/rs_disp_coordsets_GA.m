...
% Displays one or more views of a set of coordinates. Multiple views can be
% plotted in subplots of the same figure. Subplots are in 'hold on' state,
% so that one can add to the plots.
%
% Args:
%   data_in (struct): Input dataset structures, with fields `ds` (cell array
%     of coordinate structures), `sas` (cell array of metadata structures),
%     and `sets` (cell array of additional metadata structures). Typically
%     read by `rs_get_coordsets`, or created as `data_out` from
%     `rs_align_coordsets`, as `aux_out.components` from `rs_knit_coordsets`,
%     as `data_out` from `rs_knit_coordsets`, or as `data_out` from
%     `rs_xform_apply`.
%   aux (struct): Auxiliary options. See Notes for `aux.opts_disp` fields.
%
% Returns:
%   aux_out (struct): Auxiliary outputs and parameter values used, with fields
%     `warnings` (warnings generated in creating arguments for
%     `psg_get_coordsets`, or related to graphics capabilities and alpha
%     blending), `warn_bad` (int, count of warnings that prevent further
%     processing), and `opts_disp` (the input structure `opts_disp` with
%     defaults and overrides, including `opts_disp.fig_handle`, the handle
%     to the figure that was created).
%
% Notes:
%   `aux.opts_disp` fields:
%
%   Figure options:
%
...
%   - `fig_name` (str): title for figure.
%
%   Axis options:
%
%   - `axis_handles` (handle): handle to axes, one for each subplot, will be created if not supplied.
%   - `axis_font_size` (int): font size, defaults to 8 (modifiable in `rs_aux_defaults_define`).
%   - `axis_label_prefix` (str): prefix for axis label, defaults to `'dim'` (modifiable in `rs_aux_defaults_define`).
%   - `axis_label_font_size` (int): font size, defaults to `axis_font_size`.
%   - `axis_labels` (cell array of str): text for axis labels, cycled through if necessary. If empty, labels are generated from `axis_label_prefix`.
%   - `axis_view`: 3-d view of axis, as cell array for each subplot. Can be 2, 3 (default), or an azimuth-elevation pair, where `[AZ,EL]=[-37.5,30]` is the default 3-D view.
%   - `axis_equal` (int): set to 1 (default) to set axes to have equal scales.
%   - `axis_range` (str): `'tight'` (default), `'auto'` (Matlab's automatic scaling), or `'list'` (given by `axis_range_list`).
%   - `axis_range_list`: list of `[low, high]` values, one for each coordinate plotted, cycled through by rows if necessary.
%
%   Dimension and dataset selection:
%
%   - `dim_select` (int): dimension to display, i.e., `data_in.ds{set_select}{dim_select}`, defaults to 3 unless only two dims are available.
%   - `set_select` (array): datasets to show, defaults to `[1:length(data_in.da)]`.
%   - `coord_group_size` (int): number of coords to display together, in range `[2 3]`, defaults to `min(dim_select, max(number of dimensions available in all sets))`.
%   - `coord_group_method` (str): method of selecting coordinates, corresponds to `opts_vis.which_dimcombs` in `psg_visualize`. Options are `'all'` (default, plot all combinations), `'keeplow'`, `'keepone'`, `'rolling'`, `'onlylowest'`, or `'list'` (specify a list in `opts_disp.coord_groups`).
%   - `coord_groups` (array): coordinates to show in groups, array of integers in range `[1 dim_select]`; each row will generate one panel.
%
%   Set style options:
%
%   - `set_labels`: labels for each dataset in legend, defaults to `'set 1'`, etc. Text string or cell array of strings.
%   - `set_colors`: color assigned to each set, defaults to `{'k','b','c','m','r',[0.5 0.5 0],'g'}`. Can be an RGB triplet.
%   - `set_markers`: marker assigned to each set, defaults to `{'.'}`.
%   - `set_markersizes` (int): marker size assigned to each set, defaults to 8.
%   - `set_filled` (int): 1 for filled markers, 0 for unfilled (default). Only some symbols (e.g., `o`, `s`, `h`, `p`) can be unfilled.
%   - `set_colors_filled`: color assigned to fill, ignored if `set_filled=0`, defaults to `set_colors`.
%   - `set_alphas` (float): alpha blending for each set, defaults to 1 (may be system-dependent, see `aux_out.warnings`).
%   - `set_offsets`: additive offset for plotting data from each set, defaults to `zeros(1,dim_select)`. May be specified by rows of length `dim_select`, or as `'margin_amount'` or `'margin_fraction'`.
%   - `set_offsets_margin_amount`: absolute margin between datasets, defaults to `ones(1,dim_select)`. See `set_offsets`.
%   - `set_offsets_margin_fraction`: fractional margin between datasets, defaults to `zeros(1,dim_select)`. See `set_offsets`.
%   - `set_offsets_coordchoices`: specifies which coordinate is offset when `set_offsets='margin_amount'` or `'margin_fraction'`. Can be `'first'`, `'last'`, `'all'`, a subset of `[1:dim_select]`, or a cell array of subsets.
%   - `set_tags`: the `'tags'` field applied to each plot, defaults to `'set 1'`, etc. Text string or cell array of strings.
%
%   Data display options:
%
%   - `data_show_method` (str): which data points to show, `'all'` (default), `'none'`, `'first'`, `'last'`, or `'list'`.
%   - `data_show_list`: list of data points to show if `data_show_method='list'`.
%   - `data_label_method` (str): which data points to label, `'all'` (default), `'none'`, `'first'`, `'last'`, or `'list'`.
%   - `data_label_list`: list of data points to label if `data_label_method='list'`.
%   - `data_label_setsel_method` (str): which sets to label, `'all'`, `'none'`, `'first'` (default), `'last'`, or `'list'`.
%   - `data_label_setsel_list`: list of datasets to label if `data_label_setsel_method='list'`.
%   - `data_label_font_size` (int): font size for data labels, defaults to `axis_font_size`.
%   - `data_label_interpreter` (str): interpreter for labeling data, empty (default) is system default, alternatively `'none'`, `'tex'`, or `'latex'`.
%
%   Callout options:
%
%   - `callout_amount` (float): amount to expand label position from the data (in units of rms of data from centroid), defaults to 0.
%   - `callout_colors`: color for callout lines, defaults to `{'k'}`. Can also be `'set_colors'` to match `set_colors`.
%   - `callout_linestyles`: line style for callouts, defaults to `{'-.'}`.
%   - `callout_linewidths` (float): width for callouts, defaults to 1.
%
%   Data connection options:
%
%   - `connect_data_method` (str): which pairs of data points to connect within a set. Options are `'none'` (default), `'all'`, `'star'` or `'star_first'`, `'star_last'`, `'chain'`, `'circuit'`, or `'list'`.
%   - `connect_data_list`: two-column array of data points to connect if `connect_data_method='list'`.
%   - `connect_data_linestyles`: line styles for connections within each set, defaults to `{'none'}` (disconnected).
%   - `connect_data_linewidths` (float): line widths for connections within each set, defaults to 1.
%
%   Set connection options:
%
%   - `connect_sets_method` (str): which sets to connect, `'none'` (default), or any of the options in `connect_data_method`.
%   - `connect_sets_list`: two-column array of sets to connect if `connect_sets_method='list'`.
%   - `connect_sets_data_method` (str): which data points to connect between sets, `'all'` (default), any of the options in `data_label_method`, or `'labeled'`.
%   - `connect_sets_data_list`: list of data points to connect between sets if `connect_sets_data_method='list'`.
%   - `connect_sets_color_mode` (str): how connection line is colored: `'first'`, `'last'`, `'split'` (default), or `'list'`.
%   - `connect_sets_linestyles`: line styles assigned to each set, defaults to `'-'`.
%   - `connect_sets_linewidths` (float): line widths assigned to each set, defaults to 1.
%
%   Legend and display options:
%
%   - `if_box` (int): set to 1 (default) to include a box in a 3d plot.
%   - `if_grid` (int): set to 1 (default) to include the grid.
%   - `if_legend` (int): 1 (default) to include legend, 0 to omit, -1 to omit from subplots but add an extra subplot with a legend.
%   - `legend_font_size` (int): defaults to `axis_font_size`.
%   - `legend_location` (str): defaults to `'Best'`.
%   - `legend_interpreter` (str): interpreter for set label in legend, empty (default) is system default, alternatively `'none'`, `'tex'`, or `'latex'`.
%   - `legend_tags`: cell array or single string that must be present at the start of a tag for inclusion in legend, defaults to `'set'`.
%   - `if_warn` (int): set to 1 to display warnings related to plot configurations.
%   - `if_finalize` (int): set to 1 (default) to finalize axis, view, and legend.
%
%   Consistency check options:
%
%   - `opts_check.if_warn` (int): set to 1 (default) to show warnings when datasets are checked for consistency.
%
% See also:
%   RS_CHECK_COORDSETS, RS_GET_COORDSETS, RS_ALIGN_COORDSETS,
%   RS_KNIT_COORDSETS, RS_XFORM_APPLY, PSG_VISUALIZE, PSG_PLOTCOORDS,
%   RS_PLOT_STYLE.
if (nargin<=1)
    aux=struct;
end
