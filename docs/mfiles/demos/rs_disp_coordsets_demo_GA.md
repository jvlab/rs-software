...
Demonstrate display of several datasets and their consensus.
Data are plotted with and without rotating to consensus

This demo illustrates the following plot options:
...
See also:  [rs_disp_coordsets](rs_disp_coordsets.md), [rs_concat_coordsets](rs_concat_coordsets.md), [rs_disp_coordsets_test1](rs_disp_coordsets_test1.md), [rs_disp_coordsets_test2](rs_disp_coordsets_test2.md), [rs_disp_coordsets_test3](rs_disp_coordsets_test3.md)

## Reading datasets
Here we read four different example datasets of animals, from X et al. (XXXX)
...
We read the coordinates of each dataset with `rs_get_coordsets`
...
Create a list of labels for each stimulus
...
Setting default values for the figure
...
### Iterating through the different sets
...
### Transformations
Align data with `rs_align_coordsets`, rotate data into a consensus with `rs_knit_coordsets`,
and use each component, aligned to consensus, for further plotting
...
### Concatenation
Concatenate the component data and the consensus with `rs_concat_coordsets`

```matlab
data_cons=rs_concat_coordsets(aux_knit.components,data_consensus);
```

and displaying with `rs_disp_coordsets`
